...
pub fn diff_by_key<S, T, K: Eq + Hash>(
    ...
    if a.peek().is_none() {
        return b.for_each(|x| cb.inserted(x)); // If all of a was synced, add remaining in b
    ...
    if moved {
        ...
        let mut seq = sources
            ...
            .into_iter()
            .peekable();
        for (i, (b, source)) in b.into_iter().zip(sources).enumerate() {
        ...
        for (b, source) in b.into_iter().zip(sources) {
            if let Some((_, a)) = source {
                cb.unchanged(a, b);
            } else {
                cb.inserted(b);
