...
use super::{event::BoxedHandler, fragment::PersistedFragment, utils, HtmlMount};

#[derive(Clone, Debug)]
pub struct Html;

pub struct HtmlState {
	pub fragment: PersistedFragment,
...
impl Target for Html {
	...
	fn mount(ctx: Mount<Html>) -> Result<(Self::Result, Mount<Html>), Self::Error> {
		...
		if target.is_none() {
			*target = Some(HtmlState { nodes, handlers })
		} else {
			let rt = target.as_mut().unwrap();
			if rt.nodes.len() > 0 {
